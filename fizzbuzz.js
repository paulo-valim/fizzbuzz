...
function fizzBuzzIt() {
    let playerInput = parseInt(document.getElementById("input").value);
    let printResult = document.getElementById("result");
  
    //Clean the previus result before executing the function again:
    printResult.innerHTML = "";
  
    //Verify all the numbers up to the input:
    for (let i = 1; i <= playerInput; i++) {
      if (i % 3 == 0 && i % 5 != 0) {
        printResult.innerHTML += "<strong>Fizz</strong>, ";
      } else if (i % 5 == 0 && i % 3 != 0) {
        printResult.innerHTML += "Buzz, ";
      } else if (i % 15 == 0) {
        printResult.innerHTML += "Fizz Buzz, ";
      } else {
        printResult.innerHTML += i + ", ";
      }
    }
  
    console.log(printResult.innerHTML);
    console.log(printResult.textContent);
  
    //let finalResult = printResult.textContent;
    let finalResult2 = printResult.innerHTML.slice(
      0,
      printResult.innerHTML.length - 2
    );
    printResult.innerHTML = finalResult2;
  }
